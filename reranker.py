...
class Reranker:
    ...
    def rerank_docs(
        ...
        try:
            ...
            # threshold기준으로 필터링
            threshold = 0.2
            reranked_results = [r for r in filtered_results if r["relevance_score"] >= threshold]
            ...
            if "quota" in str(e).lower() or "retry" in str(e).lower():
                ...
                if retry_count < max_retries:
                    ...
                    logger.warning(f"리랭킹 중 쿼터 초과, {wait_time}초 후 재시도합니다... ({retry_count + 1}/{max_retries})")
